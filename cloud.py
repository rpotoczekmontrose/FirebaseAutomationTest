...
def export_documents(project_id="testproject-c1950"):
    ...
    request = firestore_admin_v1.ExportDocumentsRequest(
        name=f"projects/{project_id}/databases/(default)", output_uri_prefix="gs://testproject-c1950.appspot.com"
    )
...
def import_documents(project_id="terra-scouts-us", backup_location=""):
    ...
    token = subprocess.check_output(["gcloud", "auth", "print-access-token"], shell=True).decode(sys.stdout.encoding).strip()
...
cred = credentials.Certificate('C:\\Users\\Montrose\\EverScouts\\es-react-native\\terraform\\terra-scouts-us-51a568462499.json')
terra_app = firebase_admin.initialize_app(cred, name="backup", options={"projectId":"testproject-c1950", "storageBucket":"testproject-c1950.appspot.com"})
...
terra_app = firebase_admin.initialize_app(cred, name="worker", options={"projectId":"terra-scouts-us"})
